...
$$
D_{KL}(p(x)\parallel q(x)) = \mathbb{E}_{p(x)}\left[ \log{\frac{1}{q(x)}} \right] - \mathbb{E}_{p(x)}\left[ \log{\frac{1}{p(x)}} \right] = \mathrm{C.E.}(p, q) - H(p)
$$
...
$$
D(p\parallel q)\ge 0, \mathrm{with equality iff } p(x)=q(x)
$$
...
D(p\parallel q) &= \sum _{x}p(x)\left(-\log{\frac{q(x)}{p(x)}}\right) \\
 & \ge -\log\left( \sum_{x}p(x){\frac{q(x)}{p(x)}}\right) (\because \mathrm{Jensen inequality})\\
 & =-\log \sum_{x}q(x)=0
\end{align}
$$
